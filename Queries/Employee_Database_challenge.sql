...
-- Additional analysis: Retrieve number of employees by 
-- most recent title for mentorship eligibility born in 1965
...
-- Additional analysis: Retrieve number of employees by 
-- most recent title for mentorship eligibility born in 1962-1965
SELECT DISTINCT ON (emp_no)
	e.emp_no,
	e.first_name,
	e.last_name,
	...
	de.to_date,
	t.title
INTO mentorship_eligibility_multiyear
FROM employees as e
	INNER JOIN dept_emp as de
		ON e.emp_no = de.emp_no
	INNER JOIN titles as t
...
SELECT * FROM mentorship_eligibility_multiyear

SELECT COUNT(mem.emp_no), mem.title
INTO mentorship_titles_multiyear
FROM mentorship_eligibility_multiyear as mem
GROUP BY title
ORDER BY COUNT(title) DESC;

SELECT * FROM mentorship_titles_multiyear
